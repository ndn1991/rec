...
object Utils {
  ...
  def saveToCSD(rows: RDD[(Long, Array[(Long, Double)])], keySpace: String, tableName: String, cqlCreates: List[String], someColumns: SomeColumns): Unit = {
    println(s"saveToCSD keySpace($keySpace) + tableName($tableName) + someColumns($someColumns)")
    CassandraConnector(rows.sparkContext.getConf).withSessionDo { session =>
      cqlCreates.foreach { cql => session.execute(cql)}
    }
    rows.map(v => (v._1, v._2.map(_._1).toList, v._2.map(_._2).toList)).saveToCassandra(keySpace, tableName, someColumns)
  }
