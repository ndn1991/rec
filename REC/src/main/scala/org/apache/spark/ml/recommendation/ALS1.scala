...
import com.ndn.spark.mlib.optimization.NNLS1
import com.ndn.spark.util.collection.{SortDataFormat1, Sorter1}
...
object ALS1 extends Logging {
  ...
  private[recommendation] class NNLSSolver extends LeastSquaresNESolver {
    ...
    private var workspace: NNLS1.Workspace = _
    ...
    private def initialize(rank: Int): Unit = {
      if (!initialized) {
        this.rank = rank
        workspace = NNLS1.createWorkspace(rank)
    ...
    override def solve(ne: NormalEquation, lambda: Double): Array[Float] = {
      ...
      val x = NNLS1.solve(ata, ne.atb, workspace)
      ne.reset()
      x.map(x => x.toFloat)
    }
  ...
  private[recommendation] class UncompressedInBlock[@specialized(Int, Long) ID: ClassTag](
    ...
    private def sort(): Unit = {
      ...
      val sorter = new Sorter1(new UncompressedInBlockSort[ID])
  ...
  /**
   * [[SortDataFormat1]] of [[UncompressedInBlock]] used by [[Sorter1]].
   */
  private class UncompressedInBlockSort[@specialized(Int, Long) ID: ClassTag](
                                                                               implicit ord: Ordering[ID])
    extends SortDataFormat1[KeyWrapper[ID], UncompressedInBlock[ID]] {
   ...
   * Caching of the input factors is handled in [[ALS1#train]].
   */
