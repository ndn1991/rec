 ...
 */
package com.adr.bigdata.search.handler.response;

import com.adr.bigdata.search.handler.query.cmbackend.CmBackendUtils;
...
public class FrontEndJsonGroupWriter implements QueryResponseWriter {
	...
	public void write(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp)
			throws IOException {
		JSONGroupWriter w = new JSONGroupWriter(writer, req, rsp);
		w.setFieldNameConverter(fieldNameConverter);
		try {
			w.writeResponse();
		} finally {
			w.close();
...
class JSONGroupWriter extends TextResponseWriter {
	...
	public JSONGroupWriter(Writer writer, SolrQueryRequest req,
			SolrQueryResponse rsp) {
		super(writer, req, rsp);
		namedListStyle = req.getParams().get(JSON_NL_STYLE, JSON_NL_FLAT)
				.intern();
	...
	public void setFieldNameConverter(NamedList fieldNameConverter) {
		this.fieldNameConverter = fieldNameConverter;
	}

	private String convertFieldName(String name) {
		String value = (String) fieldNameConverter.get(name);
	...
	public SolrDocumentList parseCategoryList(DocList ids) throws IOException {
	...
	public void writeSolrDocument(String name, SolrDocument doc,
		...
		for (String fname : doc.getFieldNames()) {
			...
			indent();
			writeKey(convertFieldName(fname), true);
			// writeKey(fname, true);
			...
			if (val instanceof List) {
				...
				writeArray(name, ((Iterable) val).iterator());
                            ...
                            if (fname.equals("create_time")) {
                                IndexableField time = (IndexableField) val; 
                                writeVal(fname, CmBackendUtils.getDateFrom(time.numericValue().longValue()));
                            }else{
				writeVal(fname, val);
                            }
			}
	...
	@Override
	public void writeStartDocumentList(String name, long start, int size,
...
abstract class NaNFloatGroupWriter extends JSONGroupWriter {

	abstract protected String getNaN();

	abstract protected String getInf();

	public NaNFloatGroupWriter(Writer writer, SolrQueryRequest req,
			SolrQueryResponse rsp) {
		super(writer, req, rsp);
	}
